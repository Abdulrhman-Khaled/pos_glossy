  ...
  <div>
    ...
    <v-card style="max-height: 90vh; height: 90vh" class="cards my-0 py-0 mt-3 grey lighten-5">
      ...
      <v-row align="center" class="items px-2 py-1 mt-0 pt-0" v-if="pos_profile.posa_use_delivery_charges">
        <v-col cols="6" class="pb-0 mb-0 pr-0 pt-0">
        ...
        <v-col cols="4" class="pb-0 mb-0 pt-0">
          <v-text-field dense outlined color="primary" :label="frappe._('Delivery Charges Rate')"
            ...
            :prefix="currencySymbol(pos_profile.currency)" v-model="delivery_charges_rate_input"
            @change="update_delivery_charges_rate()"></v-text-field>

        </v-col>
        <v-col cols="2" class="pb-0 mb-0 pt-0">
          <v-btn block class="pa-0" color="success" dark @click="update_delivery_charges_rate_api">{{ __("Save")
          }}</v-btn>
        </v-col>
      ...
      <v-row no-gutters>
        <v-col cols="7">
          <v-row no-gutters class="pa-1 pt-9 pr-1">
            ...
            <v-col cols="6" class="pa-1 mt-2">
              <v-text-field v-model="total_input" :value="formtCurrency(subtotal)"
                @change="update_discount_subtotal(total_input)" :prefix="currencySymbol(pos_profile.currency)"
                :label="frappe._('Total')" outlined dense hide-details color="success"></v-text-field>
            </v-col>
          </v-row>
        </v-col>
        <v-col cols="5">
...
export default {
  ...
  computed: {
    ...
    subtotal() {
      ...
      this.total_input = this.flt(sum, this.currency_precision);
      return this.flt(sum, this.currency_precision);
    },
  ...
  methods: {
    ...
    update_discount_subtotal(total_input) {
      const value = flt(total_input);
      if (value) {
        this.discount_amount = (this.Total - value);
      } else {
        this.additional_discount_percentage = 0;
        this.discount_amount = 0;
      }
    ...
    calc_prices(item, value, $event) {
      if (event.target.id === "rate") {
        ...
        if (value < 0) {
          ...
          item.rate = ((flt(item.price_list_rate) * item.qty) - flt(value)) / item.qty;
          item.discount_percentage = 0;
        }
        ...
        if (value < 0) {
          ...
          item.rate = this.flt(
            ((flt(item.price_list_rate) * item.qty) - (((flt(item.price_list_rate) * item.qty) * flt(value)) / 100))
            / item.qty,
            this.currency_precision
          );
          item.discount_amount = this.flt(
