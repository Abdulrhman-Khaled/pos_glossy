      ...
      <v-row class="items px-2 py-1">
        ...
        <v-col cols="12" class="pt-0 mt-0">
          <div fluid class="items" v-if="items_view == 'card'">
            ...
            <div dense class="overflow-y-auto" style="max-height: 67vh; overflow-y: auto">
              <div v-for="(items, groupName) in grouped_items" :key="groupName" class="mb-4">
                <v-subheader class="text-subtitle-1 font-weight-bold grey--text text--darken-2">
                  {{ groupName }}
                </v-subheader>
                <v-row dense>
                  <v-col v-for="(item, idx) in items" :key="item.item_code + idx" xl="2" lg="3" md="6" sm="6" cols="6"
                    min-height="50">
                    <v-card hover="hover" @click="add_item(item)">
                      <v-img :src="item.image ||
                        '/assets/posawesome/js/posapp/components/pos/placeholder-image.png'
...
export default {
  ...
  methods: {
    ...
    enter_event() {
      ...
      const qty = this.get_item_qty(this.first_search, new_item);
      console.log(new_item, "new_item")
      new_item.qty = flt(qty, 3);
      new_item.item_barcode.forEach((element) => {
        if (new_item.custom_use_pos_scale_code) {
        ...
        if (!new_item.custom_use_pos_scale_code) {
          if (element.barcode == this.mainSearch) {
            new_item.uom = element.posa_uom;
            match = true;
          }
        }
    ...
    get_item_qty(first_search, new_item) {
      let scal_qty = Math.abs(this.qty);
      if (first_search.startsWith(this.pos_profile.posa_scale_barcode_start) && new_item.custom_use_pos_scale_code) {
        // get last 5 digits
        let pesokg1 = first_search.substr(first_search.length - this.pos_profile.weight_digits, first_search.length);
        let pesokg;
    ...
    get_search(first_search) {
      ...
      if (
        ...
        search_term = first_search.substr(this.pos_profile.posa_scale_barcode_start.toString().length, first_search.length - this.pos_profile.weight_digits - this.pos_profile.posa_scale_barcode_start.toString().length);
  ...
  computed: {
    filtred_items() {
      this.mainSearch = this.first_search;
        ...
        if (!this.search || this.search.length < 3) {
          ...
          filtred_list = filtred_group_list.filter((item) => {
            ...
            for (let element of item.item_barcode) {
              if (item.custom_use_pos_scale_code) {
                if (element.barcode == this.search) {
                  console.log("element.barcode, this.search")
                  console.log(element.barcode, this.search)
                  found = true;
                  break;
                }
              ...
              if (!item.custom_use_pos_scale_code) {
                if (element.barcode == this.mainSearch) {
                  console.log("element.barcode, this.mainSearch")
                  console.log(element.barcode, this.mainSearch)
                  found = true;
                  break;
                }
